...
$(document).ready(function(){
    ...
    //No conviene inicializarlo aqui
    //console.log('minimap renderMinimaView prebind');
    $(document).bind('xhelos.map.move',function(e,data){
        //console.log(data,'map.move parameter');
        renderMinimapViewPort(data);
    //renderArmyInMinimap();
    });
    
    //Importante, se hace otro click para que recien ahora se haga el bindeo
    //console.log(xeno['battle']['phase'],'phase minimap');
    //console.log(xeno['battle']['phase'],'phase minimap');
    if( (exist(xeno['battle']['phase'])) && (xeno['battle']['phase']=='B') ){
    //selectArmy(getActualArmyId()); //No es necesario, pero.....
    }
...
function renderMinimapViewPort(position){
    ...
    if(!domExist('#MinimapViewPort')){
        ...
        $('#MinimapViewPort').draggable({
            containment: '#Minimap',//LIMITES DE MINIMAP
...
console.log("x="+x+"y="+y);
            ...
            if(x < 0){
                //console.log("x("+x+")");
                //console.log("x("+y+")");
                $( "#BattleMap" ).css('left',x+'px');//posiciones viewmap
            ...
            if(y < 0){
                ...
                $( "#BattleMap" ).css('top',y+'px');//posiciones viewmap
            ...
            if(x > limiteAncho){
                ...
                $( "#BattleMap" ).css('left',(-1)*limiteAncho+'px');//posiciones viewmap
                //console.log("x="+x+"y="+y)  ;
                //x = -1*1200;
                //posiciones viewmap
                //$( "#BattleMap" ).css('top',1200);//posiciones viewmap
                //console.log("left:"+$( "#BattleMap" ).css('left'));
            ...
            if(y > limiteAltura ){
                ...
                $( "#BattleMap" ).css('top',(-1)*limiteAltura+'px');//posiciones viewmap
                    
                return false;
            } 
            
                 
            //x mas el tamaño del miniview todo entre tamaño del miniview
