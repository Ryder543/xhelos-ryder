...
class perlinnoise {
    ...
    function noise($x, $y, $z, $size=NULL) {
        ...
        //debug::log( ($value / $initialSize),'VALOR RETORNADO');
...
/*
  ...
  echo '<div style="position:absolute; background-color:#'.$num.$num.$num.'; width:'.$size.'px; height:'.$size.'px; top:'.($y*$size).'px; left:'.($x*$size).'px"></div><!-- '.$raw.' -->
  ';
  }
  }
  // */
