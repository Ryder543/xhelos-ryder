...
class executearmyactionwork extends work {
    ...
    public function execute() {
        ...
        if (!$reactionTD->isValid()) {
            ...
            $regionState = $this->getJax()->getOState();
    ...
    private function validateArmy(){
        ...
        debug::log($region_id,"executeaiactionwork->validateArmy");
        $ox = $this->getParam("ox");
        $oy = $this->getParam("oy");
        $armyman = armyman::singleton();
        $army_origin = $armyman->findByCoord($region_id, $ox, $oy);
        $player_id = $this->getCurrentPlayer()->getId();
        //$army_origin->initByCoord($region_id,$ox,$oy);
        if (($ok) AND !($army_origin->exist())) {
            debug::error('2 No hay ningun army en la posicion indicada');
            $this->setBadDt('2 No hay ningun army en la posicion indicada');
            $ok = false;
        }
    ...
    private function validateMana(){
        ...
        if ($ok) {
            ...
            if (!$manaDT->isOk()) {
                debug::warning($manaDT->getText(), 'ajaxRegion->action->actionman->dt_mana');
